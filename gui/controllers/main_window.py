...
<ul>
  ...
  <li><b>Labeling Phase</b> — manually label an existing video to generate a training dataset (a folder of labelled frames that YOLO uses to learn).</li>
...
<p>Manually label a recorded video to produce a YOLO-Pose training dataset
...
(70 / 20 / 10 split). A <code>data.yaml</code> file is also written — this is a small
configuration file that tells the YOLO trainer the paths to the image folders and the list
of behaviour class names. It is generated automatically from your current label settings
when you click <b>Finish and Generate Dataset</b>.</p>
...
<h2>Train</h2>
<p>Fine-tune the YOLO-Pose model on your labelled dataset (the folder generated by Labeling Phase).</p>
<ul>
  <li>Select the <b>dataset folder</b> (the labelled images folder that contains <code>data.yaml</code>).</li>
  <li>Select or confirm the <b>data.yaml</b> path (auto-detected from the dataset folder; lists the class names and image paths for the trainer).</li>
...
This creates the dataset folder (labelled images split into train / valid / test) and a
<code>data.yaml</code> file with the class names taken from your current label settings.
Repeat for more videos — frames accumulate in the same dataset folder.</p>

<h3>How do I train the model on my dataset?</h3>
<p>Go to <b>Train</b>, point it at the dataset folder (the one containing <code>data.yaml</code>
and the <code>train/</code>, <code>valid/</code> sub-folders), give the model a name, and
click <b>Train</b>. The best checkpoint is saved to <code>models/</code> automatically.</p>
...
<ul>
  ...
  <li><b>Fase Etiquetado</b> — etiqueta manualmente un video para generar un dataset de entrenamiento (carpeta de frames etiquetados que YOLO usa para aprender).</li>
...
<p>Etiqueta manualmente un video grabado para producir un dataset de entrenamiento YOLO-Pose
...
<code>train/images</code>, <code>train/labels</code>, <code>valid/images</code>, etc.
(reparto 70 / 20 / 10). Se escribe tambien un archivo <code>data.yaml</code> — un fichero
de configuracion que indica al entrenador YOLO las rutas de las carpetas de imagenes y la
lista de nombres de clases de comportamiento. Se genera automaticamente con tus etiquetas
actuales al pulsar <b>Finalizar y Generar Dataset</b>.</p>
...
<h2>Entrenar</h2>
<p>Ajusta el modelo YOLO-Pose sobre tu dataset etiquetado (la carpeta generada por Fase Etiquetado).</p>
<ul>
  <li>Selecciona la <b>carpeta del dataset</b> (la carpeta de imagenes etiquetadas que contiene <code>data.yaml</code>).</li>
  <li>Selecciona o confirma la ruta del archivo <b>data.yaml</b> (se detecta automaticamente; contiene los nombres de clase y las rutas de imagenes para el entrenador).</li>
...
de colores de la derecha). Al terminar, pulsa <b>Finalizar y Generar Dataset</b>. Esto crea
el dataset (imagenes divididas en train / valid / test) y un archivo <code>data.yaml</code>
con los nombres de clase tomados de tu configuracion de etiquetas actual. Repite con mas
videos — los frames se acumulan en la misma carpeta de dataset.</p>

<h3>¿Como entreno el modelo con mi dataset?</h3>
<p>Ve a <b>Entrenar</b>, indica la carpeta del dataset (la que contiene <code>data.yaml</code>
y las subcarpetas <code>train/</code>, <code>valid/</code>), pon un nombre al modelo y pulsa
<b>Entrenar</b>. El mejor checkpoint se guarda en <code>models/</code> automaticamente.</p>
